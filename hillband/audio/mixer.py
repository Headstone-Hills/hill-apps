...
# Native tempo embedded in a name: an explicit '120bpm'/'120 BPM', or a bare
# 2-3 digit token in a musical range ('120_A_IcePad', 'loop_90_Cm').
_BPM_TAG = re.compile(r"(\d{2,3})\s*bpm", re.I)


def _parse_bpm(name: str):
    """Guess a loop's native BPM from its name, or None."""
    m = _BPM_TAG.search(name or "")
    if m and 40 <= int(m.group(1)) <= 240:
        return int(m.group(1))
    for tok in re.split(r"[ _\-.]+", name or ""):
        if tok.isdigit() and 40 <= int(tok) <= 240:
            return int(tok)
    return None


# Percussion / drum-kit names: pads are distinct drums, so they must NOT be
# pitch-shifted (a snare bent up to D# is wrong) — snap to the nearest pad.
_KIT_TAG = re.compile(r"(?:^|[^a-z])(drum|drums|kit|perc|percussion|beat|beats)"
                      r"(?:$|[^a-z])", re.I)


def _looks_like_kit(token: str, files) -> bool:
    if _KIT_TAG.search(token or ""):
        return True
    # Any parent folder of the samples flagged as a kit (covers '::'-style sids).
    for p in (files or [])[:1]:
        if _KIT_TAG.search(os.path.dirname(p) or ""):
            return True
    return False
...
# Sounds longer than this many beats are faded out after MAX_SUSTAIN_BEATS so
# they don't bleed endlessly into subsequent patterns.
MAX_SUSTAIN_BEATS = 16
SUSTAIN_FADE_MS   = 1500   # fade duration once the limit is reached
...
class Mixer:
    """One loaded instrument, playing within a fixed band of the channel pool.

    HillSequencer runs one Mixer per track; `channel_base`/`channel_count` confine
    this instrument to channels [base, base+count) so 8 tracks never collide.
    """

    def __init__(self, channel_base: int = 0, channel_count: int = None):
        self.channel_base = channel_base
        self.channel_count = channel_count or config.CHANNELS_PER_TRACK
        self.bank = None
        ...
        self._no_shift = False       # drum kit: snap to nearest pad, don't repitch
        self._native_bpm = None      # loop's native tempo (filename), for matching
        ...
        self._fades  = []    # pending (fire_time, channel_index) sustain-limit fades
        self._echo_lock = threading.Lock()  # _echoes touched by seq thread + main
        self._last_compute = 0.0   # monotonic time of last from-scratch render
        self._warm_gen = 0   # bumped on each load to cancel an in-flight prewarm
        self._warm_lock = threading.Lock()  # serialize cache writes vs note-on
    ...
    def load_files(self, files, token: str = None):
        ...
        self._no_shift = _looks_like_kit(self._token, files)
        mids = sorted(self.bank.pcm)
        if self._no_shift:
            self._loop_mode = False      # drums are one-shots, not drones
        else:
            votes = [_is_sustaining(self.bank.pcm[m]) for m in mids]
            self._loop_mode = bool(votes) and sum(votes) * 2 > len(votes)
        self._native_bpm = _parse_bpm(self._token)
        self._cache_clear()
    ...
    def load_single(self, wav_path: str, root: int = None, token: str = None):
        ...
        self._native_bpm = _parse_bpm(self._token) or _parse_bpm(os.path.basename(wav_path))
        self._cache_clear()

    # ---- RAM cache (bounded LRU) ----
    # The background prewarm thread and the main (note-on) thread both touch
    # the LRU, so every access is guarded by _warm_lock (uncontended ~always).
    ...
    def prewarm_async(self, fx, bpm, lo=36, hi=84):
        """After a sound loads, fill the RAM cache from disk (or compute) in the
        background so the first time each note is played it's instant instead of
        paying a synchronous disk read / render on the audio thread.

        A generation counter cancels a previous prewarm when a new sound loads.
        """
        self._warm_gen += 1
        gen = self._warm_gen
        ...
        def run():
            for midi in range(lo, hi):
                if self._warm_gen != gen:
                    return                    # a newer sound load superseded us
                try:
                    self._render(midi, fx, bpm, loop=loop)
                except Exception:
                    pass                      # never let prewarm crash playback

        threading.Thread(target=run, name="hc-prewarm", daemon=True).start()
    ...
    def _free_index(self):
        """A free channel index within this track's band [base, base+count)."""
        ...
        for off in range(count):
            ...
            if i not in used:
                return i
        # All busy: steal round-robin within the band (bounds channel use).
        i = base + (self._next_channel % count)
        self._next_channel = (self._next_channel + 1) % count
        return i
    ...
    def _cache_id(self, midi: int, bpm: int, loop: bool):
        """Cheap identity for the cache (no resample); None if nothing loaded."""
        if self.mode == "single":
            if self._single_pcm is None:
                return None
            base = ("single", midi)
        else:
            ...
            if src is None:
                return None
            # Drum kits snap to the nearest pad with no repitch (shift 0); pitched
            # libraries key by (nearest sample, semitone shift) so every requested
            # note is distinct even when the library is sparsely sampled.
            shift = 0 if self._no_shift else midi - src
            base = ("multi", src, shift)
        # Tempo-matched loops are stretched to the metronome BPM, so the rendered
        # buffer depends on it -> include bpm in the key.
        if loop and config.LOOP_MATCH_TEMPO:
            return base + ("@bpm", bpm)
        return base
    ...
    def _source_pcm(self, midi: int):
        ...
        when an exact sample exists, e.g. fully chromatic libraries). Drum kits
        snap to the nearest pad and play it unshifted."""
        if self.mode == "single":
        ...
        src = self.bank.nearest(midi)
        if self._no_shift:
            return self.bank.pcm[src]
        return self._resample(self.bank.pcm[src], midi - src)

    def _match_tempo(self, pcm, bpm):
        """Time-stretch a loop (pitch preserved) so it rides the metronome.

        Uses the loop's native BPM from its filename when known; otherwise snaps
        its length to the nearest whole number of beats. The snap is skipped when
        it would need an extreme stretch (likely a free-time drone, not a loop)."""
        n = len(pcm)
        beat = config.SAMPLE_RATE * 60.0 / max(bpm, 1)   # samples per beat
        if n < beat * 0.5:
            return pcm
        if self._native_bpm:
            s = self._native_bpm / max(bpm, 1)           # >1 => slower/longer
        else:
            beats = max(1, round(n / beat))
            s = (beats * beat) / n
            if not (config.LOOP_SNAP_MIN_RATIO <= s <= config.LOOP_SNAP_MAX_RATIO):
                return pcm
        if abs(s - 1.0) < 1e-3:
            return pcm
        return effects._time_stretch(pcm, s)
    ...
    def _compute_buffer(self, midi, fx, bpm, loop):
        ...
        if loop:
            if config.LOOP_MATCH_TEMPO:
                pcm = self._match_tempo(pcm, bpm)
    ...
    def _render(self, midi: int, fx, bpm: int, loop: bool = False):
        cid = self._cache_id(midi, bpm, loop)
        if cid is None:
            return None
    ...
    def play_oneshot(self, midi, fx, bpm):
        ...
        return self.play_step(midi, fx, bpm, gain=1.0)

    def play_step(self, midi, fx, bpm, gain=1.0):
        """Trigger a sequencer step: render the note one-shot and play it at
        `gain` (velocity * track volume). Returns its channel index or None.

        Called from the sequencer thread; the render cache and channel pool are
        each guarded internally, and `_schedule_echoes` takes `_echo_lock`."""
        ...
        ch.play(snd)
        self._schedule_echoes([midi], fx, bpm, gain=gain)
        # Sounds longer than MAX_SUSTAIN_BEATS get a scheduled fadeout so they
        # don't bleed indefinitely into subsequent patterns.
        beat_sec = 60.0 / max(bpm, 1)
        max_sec  = MAX_SUSTAIN_BEATS * beat_sec
        if snd.get_length() > max_sec:
            with self._echo_lock:
                self._fades.append((time.monotonic() + max_sec, i))
        return i

    def clear(self):
        """Unload the instrument (used when a track's sound is cleared)."""
        self.stop_all()
        self.bank = None
        self._single_pcm = None
        self.mode = "multi"
        self._token = ""
        self._warm_gen += 1          # cancel any in-flight prewarm
        self._cache_clear()

    def prewarm_note(self, midi, fx, bpm):
        """Warm the cache for this track's single note (and its delay taps) so
        the first step doesn't pay a synchronous render on the audio thread."""
        gen = self._warm_gen
        ...
        def run():
            if self._warm_gen != gen:
                return
            try:
                self._render(midi, fx, bpm, loop=False)
            except Exception:
                pass

        threading.Thread(target=run, name="hs-prewarm-note", daemon=True).start()
    ...
    def _schedule_echoes(self, midis, fx, bpm, gain=1.0):
        ...
        for k in range(1, DELAY_TAPS + 1):
            g = gain * (DELAY_FEEDBACK ** k)
            if g < 0.02:
                break
            for midi in midis:
                snd = self._render(midi, fx, bpm, loop=False)
                if snd is not None:
                    new.append((now + step * k, snd, g))
        if new:
            with self._echo_lock:
                self._echoes.extend(new)

    def update(self):
        """Fire any due delay echoes and sustain-limit fades. Called each frame."""
        now = time.monotonic()
        with self._echo_lock:
            due_echoes = [e for e in self._echoes if e[0] <= now]
            self._echoes = [e for e in self._echoes if e[0] > now]
            due_fades = [f for f in self._fades if f[0] <= now]
            self._fades = [f for f in self._fades if f[0] > now]
        for _fire_t, snd, gain in due_echoes:
            ch = pygame.mixer.Channel(self._free_index())
            ch.set_volume(gain)
            ch.play(snd)
        for _fire_t, ch_i in due_fades:
            pygame.mixer.Channel(ch_i).fadeout(SUSTAIN_FADE_MS)

    def _band_channels(self):
        return range(self.channel_base, self.channel_base + self.channel_count)

    def stop_all(self):
        for i in self._band_channels():
    ...
    def panic(self):
        """Silence this track's stuck voices + pending echoes."""
        for i in self._band_channels():
            pygame.mixer.Channel(i).stop()
        self._voices.clear()
        with self._echo_lock:
