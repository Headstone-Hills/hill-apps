"""HillBand — main renderer.

8 rows, split visually into two halves:
  * Rows 0–3 (melodic, blue accent): show note, chord type, scale mode, fx badges.
  * Rows 4–7 (drums, orange accent):  show fixed name, instrument; no chord/scale.

A thin coloured stripe on the right edge of each label column marks the section.
"""
...
from config import (
    ...
    NUM_TRACKS, NUM_STEPS, DRUM_TRACK_OFFSET,
...
def _chord_label(chord_key):
    """Short display string for the chord type."""
    if chord_key in ("unison", None):
        return ""
    return chord_key
...
class UI:
    ...
    def _draw_track_rows(self, surface, top, row_h):
        ...
        for ti in range(NUM_TRACKS):
            ...
            muted   = track["muted"]
            is_cur  = ti == self.cursor_track

            bg = CLR_VOICE_ACTIVE if is_cur else CLR_VOICE_BG
            pygame.draw.rect(surface, bg, (0, y0, SCREEN_W, row_h - 2))

            # Section accent stripe on right edge of the label column
            accent = CLR_DRUM_ACCENT if track["is_drum"] else CLR_MELODIC_ACCENT
            pygame.draw.rect(surface, accent, (TRACK_LABEL_W - 3, y0, 3, row_h - 2))

            # Multi-select marker
            if ti in self.selected_tracks:
                pygame.draw.rect(surface, CLR_CURSOR, (0, y0, 3, row_h - 2))

            self._draw_track_label(surface, ti, track, muted, is_cur, y0, row_h)
            ...
            for si in range(NUM_STEPS):
                ...
                is_cursor  = is_cur and si == self.cursor_step
                is_playhead = si == playhead and self.transport.playing

                # Faint cursor column across all rows (drawn once on row 0)
    ...
    def _draw_track_label(self, surface, ti, track, muted, is_cursor_row, y0, row_h):
        ...
        if is_drum:
            # Drum track: name + instrument on two lines.
            surface.blit(self.font_s.render(track["name"][:8], True, name_clr), (4, y0 + 2))
            instr = _short_instrument(track["sound"])
            surface.blit(self.font_s.render(instr[:16], True, CLR_TEXT_DIM), (4, y0 + 16))
        else:
            # Melodic track: name + note, chord/scale badges, fx badges.
            chord_key = track.get("chord_type", "unison")
            scale_key = track.get("scale_mode", "none")
            note_txt  = note_name(track["note"])
            chord_txt = _chord_label(chord_key)
            head = f"{track['name']} {note_txt}"
            if chord_txt:
                head += f" {chord_txt}"
            surface.blit(self.font_s.render(head[:18], True, name_clr), (4, y0 + 2))

            # Second line: instrument + scale mode
            instr = _short_instrument(track["sound"])
            if scale_key and scale_key != "none":
                instr_line = f"{instr[:10]} ~{scale_key[:8]}"
            else:
                instr_line = instr[:16]
            surface.blit(self.font_s.render(instr_line, True, CLR_TEXT_DIM), (4, y0 + 16))

            # FX badges on third line
            fx = track["effects"]
            bx = 4
            by = y0 + 30
            for on, label, clr in (
            ...
            if fx.any_active() and fx.wetdry > 0 and (fx.reverb or fx.chorus or fx.delay):
                ...
                surface.blit(s, (bx, by)); bx += s.get_width() + 3
            if fx.crushing():
                surface.blit(self.font_s.render("crsh", True, CLR_FX_CRUSH), (bx, by))
        ...
        if fillw > 0:
            pygame.draw.rect(surface,
                             CLR_STEP_MUTED_ON if muted else CLR_STEP_ON,
