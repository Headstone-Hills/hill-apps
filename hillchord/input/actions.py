...
class Actions:
    def __init__(self, state, mixer, looper, metronome, library, arp):
        ...
        self._taps = []            # recent tap-tempo timestamps (library X)
        self._sustained = set()    # note-mode voice IDs kept alive by sustain pedal

    def handle(self, frame: ch.InputFrame):
        # Cheat sheet: hold FUNCTION + press SELECT (or vice versa) opens it.
        # Kept off SELECT+B since SELECT alone is the sustain pedal and B is a
        # chord/note button — that combo fired help mid-sustain.
        fn_select_combo = (
            (bm.FUNCTION in frame.held and bm.SELECT in frame.pressed) or
            (bm.SELECT in frame.held and bm.FUNCTION in frame.pressed))
        if fn_select_combo and not self.state.help:
            self.state.help = True
            self.state.help_scroll = 0
            self._fn_used = True   # suppress the FN-release chord/note mode flip
            return

        # While the cheat sheet is open it's modal: A or B closes; D-pad scrolls.
        if self.state.help:
            from ui import help_overlay
            if bm.A in frame.pressed or bm.B in frame.pressed:
    ...
    # Sustain pedal: hold SELECT to sustain notes/chords past button release.
    ...
    # Key cycling: SELECT+R1 up, SELECT+L1 down (circle of fifths). Lives on
    # SELECT-as-modifier rather than a bare SELECT press so holding SELECT to
    # sustain doesn't also nudge the key on the same press.
    def _handle_key_cycle(self, frame: ch.InputFrame):
        if bm.SELECT not in frame.held:
            return
        s = self.state
    ...
    # Function is using D-Up/Down for octave).
    def _update_latches(self, frame: ch.InputFrame, fn: bool):
        now = time.monotonic()
        for d in (frame.pressed & (bm.DPAD)):
    ...
    def _handle_library(self, frame: ch.InputFrame):
        s = self.state
        # B: go up one level if in a subfolder, else close library.
        if bm.B in frame.pressed:
            import os
            if (self.library.filter is None and
                    os.path.abspath(self.library.cur) != os.path.abspath(self.library.root)):
                self.library.cur = os.path.dirname(self.library.cur)
                self.library.index = 0
                self.library.refresh()
            else:
                s.screen = "play"
            return
        # START falls through (not consumed here) so play/pause still works.
        ...
        if bm.X in frame.pressed:
            self.library.cycle_filter(self.state)
            return
    ...
    # Tap-tempo: tap X in the library in time; BPM is the average of the recent
    # tap intervals. A pause >2s starts a fresh count. Aligns the beat grid to
    # the last tap (so the metronome downbeat lands on your tap) unless a loop is
    # playing, whose phase we mustn't disturb.
    def _tap_tempo(self):
        s = self.state
        now = time.monotonic()
        ...
        self._taps = self._taps[-5:]          # rolling window of recent taps
        if len(self._taps) >= 2:
            intervals = [b - a for a, b in zip(self._taps, self._taps[1:])]
            avg = sum(intervals) / len(intervals)
            if avg > 0:
                bpm = int(round(60.0 / avg))
                s.bpm = max(config.BPM_MIN, min(config.BPM_MAX, bpm))
                self.looper.retime(s.bpm)
        # Anchor the grid to this tap when nothing is looping.
        if getattr(self.looper, "state", "idle") in ("idle", "recording"):
            self.metronome.transport.epoch = now
            self.metronome.reset()
