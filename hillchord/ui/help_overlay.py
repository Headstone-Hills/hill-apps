"""Cheat-sheet overlay (opened by FUNCTION+SELECT, either order). Modal +
scrollable with the D-pad. Close with A or B. Single readable column that
fits 640x480."""
...
LINES = [
    ...
    ("row", "Select (hold)", "Sustain pedal — holds notes/chords"),
    ...
    ("row", "Select + Start (hold)", "Quit (1.2s)"),
    ("head", "LOOPER (L2)"),
    ...
    ("row", "X", "Cycle filter: All/Fav/Loop/1-shot/type"),
...
def draw(surf, fonts, scroll=0):
    ...
    surf.blit(fonts["small"].render(
        "D-pad: scroll    A / B: close", True, (130, 130, 150)),
        (16, h - 24))
